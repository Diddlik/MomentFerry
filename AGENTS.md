...
- Route again: `POST /api/v1/operations/{id}/route-again` re-routes a file whose operation already reached `Completed` or `Ignored`, so a changed rename preset or destination layout can be applied. `OperationRetryService.RouteAgainAsync` marks the previous operation `Failed` with a superseded note, which is what lifts the `HasTerminalOperationAsync` block, then runs a normal transfer. The file the earlier run wrote is left in place; removing it is the user's decision. The Operations table offers it per terminal row and hides it in Dry Run. Without it the only ways to route a file again are a new event or a different source path, because `media_files` identifies a file by share plus path and neither table is ever pruned.
