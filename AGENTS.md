...
- Camera identity: phone videos usually carry no `Make`/`Model`. The extractor also requests `AndroidManufacturer`/`AndroidModel` (the `com.android.*` QuickTime keys), `ModelName` (Samsung's `smta` box, e.g. `SM-S931B`) and `Author` (Samsung's `udta/auth`, e.g. `Galaxy S25`). `ResolveModel` prefers `Model`, then `AndroidModel`, then the marketing name from `Author` but only when the Samsung box proves who wrote the file, because `Author` is free text everywhere else. That keeps a video named like a photo from the same phone. A non-zero ExifTool exit whose output still parses as JSON is used rather than discarded, so an unknown tag name costs a warning instead of every field.
