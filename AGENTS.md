...
- Self-output protection: before an identical destination is accepted as a duplicate, `SafeTransferService` asks `FindCompletedByDestinationHashAsync` whether a completed operation already wrote that exact content to a destination for a different media file. A hit means MomentFerry's own output arrived back on a source share, so the operation is `Quarantined` with the earlier destination named and the source is kept. This deliberately also catches a genuine duplicate that another device contributed, because the content hash alone cannot tell the two apart, and holding a file is recoverable while deleting it is not.
